...
int xhci_command_set_dequeue_pointer(xhci_t *xhci, int slot, int endpoint, uint64_t dequeue_ptr)
{
    xhci_trb_t *trb;
    int status;

    xhci_command_submit(&xhci->cmd, dequeue_ptr | 1, 0, (slot << 24) | (endpoint << 16) | (TRB_TYPE_STOP_ENDPOINT << 10));
...
int xhci_command_stop_endpoint(xhci_t *xhci, int slot, int endpoint)
{
    ...
    xhci_command_submit(&xhci->cmd, 0, 0, (slot << 24) | (endpoint << 16) | (TRB_TYPE_STOP_ENDPOINT << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
...
int xhci_command_reset_endpoint(xhci_t *xhci, int slot, int endpoint)
{
    ...
    xhci_command_submit(&xhci->cmd, 0, 0, (slot << 24) | (endpoint << 16) | (TRB_TYPE_RESET_ENDPOINT << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
...
int xhci_command_reset_device(xhci_t *xhci, int slot)
{
    ...
    xhci_command_submit(&xhci->cmd, 0, 0, (slot << 24) | (TRB_TYPE_RESET_DEVICE << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
...
int xhci_command_evaluate_context(xhci_t *xhci, int slot, uint64_t input_ctx_ptr)
{
    ...
    xhci_command_submit(&xhci->cmd, input_ctx_ptr, 0, (slot << 24) | (TRB_TYPE_EVALUATE_CONTEXT << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
...
int xhci_command_configure_endpoint(xhci_t *xhci, int slot, uint64_t input_ctx_ptr)
{
    ...
    xhci_command_submit(&xhci->cmd, input_ctx_ptr, 0, (slot << 24) | (TRB_TYPE_CONFIGURE_ENDPOINT << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
...
int xhci_command_address_device(xhci_t *xhci, int slot, uint64_t input_ctx_ptr)
{
    ...
    xhci_command_submit(&xhci->cmd, input_ctx_ptr, 0, (slot << 24) | (TRB_TYPE_ADDRESS_DEVICE << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
...
int xhci_command_noop(xhci_t *xhci)
{
    ...
    xhci_command_submit(&xhci->cmd, 0, 0, (TRB_TYPE_CMD_NOOP << 10));
    xhci_ring_doorbell(xhci, 0, 0);
    trb = xhci_poll_event(xhci, TRB_TYPE_COMMAND_COMPLETION);
    status = (trb->status >> 24);
